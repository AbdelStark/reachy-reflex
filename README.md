...
This is a development preview, not a hardware-tested release. The browser app now has the Reachy Mini host shell, WebRTC camera attachment, local face-box detection, an opt-in browser-microphone transcription path, a 16-signal panel, an authenticated local Jev relay, and an opt-in bounded motion adapter. Face detection, microphone access inside the host, and motion have **not** been validated on a real robot. Robot-microphone capture, VAD, direction-of-arrival, Conversation App event integration, hosted relay deployment, and calibration are still missing. No attention accuracy, end-to-end latency, cost, or human-preference result is claimed.
...
Run `npm run dev` and open `http://127.0.0.1:5173/?preview=1` for a robot-free fixture preview. Its simulated person and answers exercise the panel and policy but are not Jev or hardware results. The browser suite covers the 16 gauges, room changes, opt-in transcript path with a fake recognizer, narrow viewport, and locally served MediaPipe WebAssembly files. Unit tests cover tracking, stale observations, relay boundaries, motion gating, and transcript bounds.
